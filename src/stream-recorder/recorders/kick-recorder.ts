...
export class KickRecorder implements StreamRecorder {
  ...
  async start(channelName: string): Promise<void> {
    ...
    await this.commandBus.execute(new StreamRecordingStartCommand(channelName, PlatformName.KICK));
  }
  ...
  async stop(channelName: string): Promise<void> {
    ...
    await this.commandBus.execute(new StreamRecordingStopCommand(channelName, PlatformName.KICK));
  }

  isRecording(channelName: string): boolean {
    return this.recordingProcesses.has(this.createKey(channelName, PlatformName.KICK));
  }
  ...
  private async syncRecordingProcess(): Promise<void> {
    ...
    result.forEach((recording: RecordingProcess) => {
      this.recordingProcesses.set(this.createKey(recording.channelName, PlatformName.KICK), recording);
    });
  }
